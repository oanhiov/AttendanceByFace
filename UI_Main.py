...
class Ui_MainWindow(object):
    ...
    def __init__(self, teacher_name, teacher_id):
        ...
        sql_select_class = "SELECT maLHP FROM Lophocphans WHERE maGV = '"+self.teacher_id+"'; "
        sql_select_id_name_class = "SELECT maLHP, tenHP FROM Lophocphans INNER JOIN Hocphans ON Lophocphans.maHP =" \
                                   " Hocphans.maHP WHERE Lophocphans.maGV = '"+self.teacher_id+"';"
        ...
        self.btn_diemdanh.clicked.connect(self.Log_data)
        conn.close()

    def Log_data(self):
        currentIndex = self.listWidget.currentRow()
        print(self.listWidget.item(currentIndex).text())
...
if __name__ == "__main__":
    ...
    UI = Ui_MainWindow('Vi Văn Oanh', '003')
    UI.MainWindow.show()
    sys.exit(app.exec_())