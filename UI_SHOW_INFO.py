...
class Ui_MainWindow(object):
    ...
    #   Đánh số đi học
        ...
        # print(day, month)
        if time.localtime(time.time()).tm_mon >= month and time.localtime(time.time()).tm_mday > day:
        #     pass
        # if (time.localtime(time.time()).tm_mday <= day) and (time.localtime(time.time()).tm_mon <= month):
